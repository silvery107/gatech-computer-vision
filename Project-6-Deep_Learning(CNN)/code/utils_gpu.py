    ...
    def train_val(self):
        for epoch in range(self.start_epoch, self.config['n_epochs']):
            ...
            self.vis.line(X=np.asarray([epoch]),
                          ...
                          update='append',
                          env=self.vis_env)
            self.vis.line(X=np.asarray([epoch]).astype('float32'),
                          Y=np.asarray([top1_prec]).astype('float32'),
                          win=self.top1_win,
            ...
            self.vis.line(X=np.asarray([epoch]).astype('float32'),
                          Y=np.asarray([top5_prec]).astype('float32'),
                          win=self.top5_win,
...
def step_feedfwd(data, model, target=None, loss_fn=None, optim=None, train=True):
    """
