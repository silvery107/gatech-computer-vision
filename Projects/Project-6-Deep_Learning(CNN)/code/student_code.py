...
def create_datasets(data_path, input_size, rgb=False):
    ...
    train_data_tforms.append(transforms.Normalize(train_mean,train_std,inplace=True))
    ...
    # test_mean, train_std, or test_std values that are already calculated for
    # you. Which mean and std should you use to normalize the data?
    test_data_tforms.append(transforms.Normalize(test_mean,test_std,inplace=True))
...
class SimpleNet(nn.Module):
	...
	"""
    def __init__(self, num_classes, droprate=0.3, rgb=False, verbose=False):
        """
        ...
        self.features = nn.Sequential(
            nn.Conv2d(in_channels, 64, kernel_size=3, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(64),
        ...
        self.classifier = nn.Conv2d(in_channels=256,
...
def custom_part1_trainer(model):
    ...
    return {'loss_function':loss_function,'optimizer':optimizer,'lr_scheduler':lr_scheduler,'params':params}
...
def create_part2_model(model, num_classes):
    ...
    new_layer.weight.data.normal_(0, 1)
    new_layer.weight.data.mul_(1e-2)
    if new_layer.bias is not None:
        nn.init.constant_(new_layer.bias.data, 0)
